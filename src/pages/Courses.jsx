...
export default function Courses() {
  const { userState, enrollCourse, unenrollCourse } = useUser();
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCompetency, setSelectedCompetency] = useState('all');
  const [selectedProvider, setSelectedProvider] = useState('all');
  const [selectedCourse, setSelectedCourse] = useState(null);

  const enrolledIds = userState.enrolledCourses || ['crs_igot_102', 'crs_igot_103'];
  const userWeakAreas = (userState.diagnosticResult?.weakAreas || []).map(w => w.toLowerCase());
  ...
  const dynamicCoursesList = initialCourses.map(c => {
    const isEnrolled = enrolledIds.includes(c.id);
    const progress = userState.courseProgressMap?.[c.id] || (isEnrolled ? 45 : 0);
    const isRecommended = userWeakAreas.some(w =>
      c.competency.toLowerCase().includes(w) ||
      c.title.toLowerCase().includes(w) ||
      w.includes(c.competency.toLowerCase())
    );
  ...
  const filteredCourses = dynamicCoursesList.filter(c => {
  ...
  return (
    <PageContainer>
      <div className="space-y-8 pb-12">
        ...
        {/* Filter Controls Bar */}
        <div className="p-4 rounded-2xl bg-[#111A28] border border-[#243247] flex flex-col md:flex-row items-center justify-between gap-4 shadow-[0_0_15px_rgba(6,182,212,0.08)]">
          {/* Search Bar */}
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="w-full bg-[#070A0F] border border-[#243247] rounded-xl pl-9 pr-4 py-2 text-xs text-slate-200 focus:outline-none focus:border-cyan-500 transition-colors"
            />
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto flex-wrap">
            <div className="flex items-center gap-2 text-xs text-slate-400">
              ...
              <select
                ...
                className="bg-[#070A0F] border border-[#243247] rounded-lg px-2.5 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-cyan-500"
              >
